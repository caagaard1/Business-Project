...
if input("Do you want to recalibrate ARIMA? (y/n): ").lower() == 'y':
    ...
    best_model = sm.tsa.ARIMA(new_df['price'],
                                         order=best_order,
                                         freq='D',
                                         enforce_invertibility=True,
                                         enforce_stationarity=True)
...
new_df['original_price'] = new_df[['price']]
new_df['trans_SARIMA_fitted'] = pd.DataFrame(new_df['SARIMA_fitted'])
new_df['trans_residuals'] = new_df['original_price'] - new_df['trans_SARIMA_fitted']
new_df.to_csv('SVJ_data_SARIMA_no_transform.csv', index=True)
