...
full_data = full_data.join(futures_df)
...
def initialize_multistep(target_date, x = None):
    ...
    for i in range(60):
        ...
        ARIMA_value =  float(price_dataframe['M+1'].iloc[-1])
        ...
        temp_row  = pd.DataFrame(
            {price_dataframe.columns[0] : [jump_adj_price],
             ...
             price_dataframe.columns[4] : [jump_adj_price]}
...
def one_period_simulator(target_date):
    ...
    for x in range(100):
        one_period_simulation[f'sim {x}'] = initialize_multistep(target_date)['M+1']
...
def historical_futures_pdf():
    ...
    for i in simulation_days:
        ...
        historical_futures_pdf_data.to_csv(f'historical_futures_pdf_data_{start_date}v2.csv', index=True)
        historical_DA_est_vol.to_csv(f'historical_DA_est_vol{start_date}v2.csv', index=True)
...
plt.plot(full_data['M+1'].loc[one_period_simulation.index.min():one_period_simulation.index.max()], label = f'Actual mean: {full_data['M+1'].loc[one_period_simulation.index.min():one_period_simulation.index.max()].mean()}')
